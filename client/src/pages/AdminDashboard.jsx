    ...
    try {
      ...
      switch (type) {
        case 'revenue': {
          ...
          setDrillDown({
            type, title: 'Subscription Revenue Breakdown', columns: ['Business', 'Owner', 'Plan', 'Monthly Fee'],
        ...
        case 'applicants': {
          const res = await api.get('/admin/applications');
          setDrillDown({
            type, title: 'All Seller Applications', columns: ['Business', 'Applicant', 'Email', 'Status', 'Applied'],
        ...
        case 'terminated': {
          ...
          setDrillDown({
            type, title: 'Terminated Accounts', columns: ['Business', 'Owner', 'Email', 'Plan', 'Date'],
        ...
        case 'sellers': {
          const res = await api.get('/admin/applications', { params: { status: 'approved' } });
          setDrillDown({
            type, title: 'Active Sellers', columns: ['Business', 'Owner', 'Email', 'Phone', 'Plan'],
        ...
        case 'users': {
          const res = await api.get('/admin/users');
          setDrillDown({
            type, title: 'All Registered Users', columns: ['Name', 'Email', 'Phone', 'Role', 'Joined'],
        ...
        case 'applicationStatus': {
          const res = await api.get('/admin/applications');
          setDrillDown({
            type, title: 'Applications by Status', columns: ['Business', 'Applicant', 'Email', 'Status', 'Date'],
        ...
        case 'subscriptions': {
          const res = await api.get('/admin/applications', { params: { status: 'approved' } });
          setDrillDown({
            type, title: 'Seller Subscriptions by Plan', columns: ['Business', 'Owner', 'Plan', 'Monthly Fee'],
            data: res.data.applications.map(a => ({ Business: a.business_name, Owner: a.user_name, Plan: (a.subscription_plan || '').charAt(0).toUpperCase() + (a.subscription_plan || '').slice(1), 'Monthly Fee': '₱' + (planPrices[a.subscription_plan] || 0).toLocaleString() }))
          });
          break;
        }
